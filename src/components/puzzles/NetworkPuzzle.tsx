...
export const NetworkPuzzle: React.FC<PuzzleComponentProps> = ({ puzzle, onComplete }) => {
  ...
  const renderScanResults = () => {
    if (!puzzle.data?.scanResults) return null;
    
    return (
      <div className="space-y-2">
        {puzzle.data.scanResults.map((result: any, index: number) => (
          <div key={index} className="grid grid-cols-3 gap-4 text-sm font-mono">
            <div className="text-yellow-300">Port {result.port}</div>
            <div className="text-green-300">{result.service}</div>
            <div className="text-gray-400">{result.banner}</div>
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Card variant="terminal" className="space-y-4">
        ...
        {puzzle.data?.scanResults && (
          <Terminal title="nmap-scan-results.txt">
            <div className="space-y-2">
              <div className="text-gray-400">$ nmap -sV target-server.com</div>
              <div className="text-gray-400 mb-3">PORT     SERVICE    VERSION</div>
              {renderScanResults()}
            </div>
          </Terminal>
        )}
      ...
      <Card variant="default" className="space-y-4">
        <h3 className="text-lg font-mono text-hacker-green">Network Analysis Terminal</h3>
        
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-mono text-gray-400 mb-2">
              Which port runs the web server and what framework is it using?
            </label>
            <input
              type="text"
              value={answer}
              ...
              placeholder="e.g., Port 3000: Express.js"
          ...
          {showHint && currentHintIndex > 0 && (
            <Terminal title="network-hint">
              <div className="space-y-2">
                <div className="text-gray-400">$ netstat --hint</div>
